...
void
conta_vizinhos (int **mundo, int **passado, int lin, int col)
{
  for (int i = 0; i < lin; i++)
    {
      for (int j = 0; j < col; j++)
	{
	  ...
	  for (int k = -1; k < 2; k++)
		...
		{
		  vizinhos +=
		    passado[(i + k + lin) % lin][(j + l + col) % col];
		}
	    }
...
void
imprime_mundo (int **matriz, int lin, int col)
{
  for (int i = 0; i < lin; i++)
    for (int j = 0; j < col; j++)
...
void
copia_mundos (int **mundo, int **passado, int lin, int col)
{
  for (int i = 0; i < lin; i++)
    for (int j = 0; j < col; j++)
